    ...
    <div className={cx('menu', 'js-menu')}>
      <ul className={cx('menu-items')}>
        {routes.filter(route => route.showOnMenu === true).map((item) => (
          ...
          <Link href={item.path} passHref><a className={cx("menu-link", "js-menu-link", {'active': router.pathname.includes(item.path)})} data-cursor-type="move-left" style={{'--menu-link-color': item.color || colors.green} as CustomCSS}>{item.text}</a></Link>
        </li>
        ))}
      </ul>
